...
* `crate_name_sep` - the separator that should be used for concatenation part of crate names: either `_` (underscore) or `-` (dash).
* `macro_idents` - identifiers that will be used for creation of macros. They are collected into an object with complex schema. Check `cookiecutter.json` for an example.
